# sim_ocean003a.jl   Wulf Luck   2023 11 22
...
# in model declaration check "tracer_advection = WENO5(grid),"

import Pkg; 

Pkg.add(url="https://github.com/CliMA/ClimaOcean.jl"); Pkg.instantiate                           # Oceananigans"); Pkg.add("CairoMakie") 
...
#u_top_bc = FluxBoundaryCondition(CuArray(τx))
v_top_bc = FluxBoundaryCondition(arch_array(arch, τy))
#v_top_bc = FluxBoundaryCondition(CuArray(τy))
...
figTresult = heatmap(Array(interior(model.tracers.T, :, :, grid.Nz)))
figTresult = heatmap(Array(interior(model.tracers.T, :, :, grid.Nz)))
vel = heatmap(Array(interior(model.velocities.u, :, :, 12)))